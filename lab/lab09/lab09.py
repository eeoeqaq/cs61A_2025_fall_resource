...
def floor_div(args):
    ...
    20
    """
    assert isinstance(args,Link)
    ans=calc_eval(args.first)
    if type(ans) is not int:
        raise TypeError
    rest=args.rest
    while isinstance(rest,Link):
        dividor=calc_eval(rest.first)
        if type(dividor) is not int:
            raise TypeError
        ans=ans//dividor
        rest=rest.rest
    return ans
...
def eval_and(expressions):
    ...
    while isinstance(expressions,Link):
        operand=calc_eval(expressions.first)
        if operand is scheme_f:
            return scheme_f
        else:
            if ans<operand:
                ans=operand
...
def eval_define(expressions):
    ...
    2
    """
    assert isinstance(expressions,Link)
    tar=expressions.first
    obj=expressions.rest
    operator = obj.first 
    if operator in OPERATORS and obj.rest is not nil:
        obj=calc_eval(obj)
    else:
        obj=operator
    bindings[tar]=obj
