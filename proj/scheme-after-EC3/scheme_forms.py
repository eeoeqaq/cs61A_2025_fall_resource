...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        args=signature.rest
        measures=expressions.rest
        func=do_lambda_form(Link(args,measures),env)
...
#从一个schemelink（Link）转换到一个lambda类的实例
...
def do_and_form(expressions, env):
    ...
    rest=expressions
    while rest.rest != nil:
        va=scheme_eval(rest.first,env)
        if is_scheme_false(va):
            return va
        rest=rest.rest
    va=scheme_eval(rest.first,env,True)
    return va
    # END PROBLEM 12
...
def do_let_form(expressions, env):
    ...
    new_expressions=expressions.rest
    while new_expressions.rest is not nil:
        scheme_eval(new_expressions.first,let_env)
        new_expressions=new_expressions.rest
    return scheme_eval(new_expressions.first,let_env,True)
