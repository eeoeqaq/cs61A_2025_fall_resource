...
def boar_brawl(player_score, opponent_score):
    ...
    # BEGIN PROBLEM 2
    opponent_score//=10
    result=abs(opponent_score%10-player_score%10)*3
    if not result:
        result=1
    return result 
    # END PROBLEM 2
...
def sus_update(num_rolls, player_score, opponent_score, dice=six_sided):
    ...
    """
    # BEGIN PROBLEM 4
    a=player_score+take_turn(num_rolls, player_score, opponent_score, dice)
    a=sus_points(a)
    return a
    # END PROBLEM 4
...
def max_scoring_num_rolls(dice=six_sided, times_called=1000):
    ...
    i,j,k,ans=1,0,0,0
    while i<=10:
        k=make_averaged(roll_dice,times_called)(i,dice)
        if ans<k:
            j=i
            ans=k
        i+=1
...
def final_strategy(score, opponent_score):
    ...
    boar+sus is a steady way to get points,so we should consider it at first
    if we are closed to goal,be discreet,using a tactic with smaller risks.
    """
    # BEGIN PROBLEM 12
    if (sus_update(0, score, opponent_score)-score)>=make_averaged(roll_dice,100)(6):
        return 0
    i=0
    while i<=6:
        if (100-score)<3.5*i-1:
            return i
        i+=1
