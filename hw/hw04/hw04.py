def shuffle(s):
    ...
    #for i in range (a):
     #   b+=[s[i],s[i+a]]
    #return b
    return [x for a in [(s[i],s[a+i]) for i in range(a)] for x in a]
...
def balanced(m):
    ...
    if is_planet(m):
        return True
    if total_mass(end(right(m)))*length(right(m))!=total_mass(end(left(m)))*length(left(m)):
        return False
    if not (balanced(end(right(m))) and balanced(end(left(m)))):
        return False
    return True
